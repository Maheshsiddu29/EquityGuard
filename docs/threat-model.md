...
| T2 | Scheduled activation passes while tx is in flight | Guard reads `Clock` at execution and rejects inside the protection window |
...
| T7 | Float representation ambiguity (`-0.0`, NaN, rounding) | Byte-level comparison on-chain; tolerance policy only in display code |
