...
const safeDecision = decideExecution({ preferred: resolved("KOx", RepresentationState.SAFE), alternative: null, reroutePolicy: { maxAdditionalCostBps: 25n }, consent: null, currentSlot: 100n, inputRaw: 1n, comparison: null, routes: NO_ROUTES });
...
test("devnet execution results cannot cite mainnet quotes or evidence, or execute on non-executable decisions", () => {
  ...
  const unknown = decideExecution({ preferred: resolved("KOx", RepresentationState.UNKNOWN), alternative: null, reroutePolicy: { maxAdditionalCostBps: 25n }, consent: null, currentSlot: 100n, inputRaw: 1n, comparison: null, routes: NO_ROUTES });
