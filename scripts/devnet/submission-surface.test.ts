...
test("the transport is imported only by the product executor and the devnet admin tooling", () => {
  ...
  assert.deepEqual(importers, ["scripts/demo/devnet-execution.ts", "scripts/devnet/cli.ts", "scripts/devnet/scenarios.ts"]);
});
